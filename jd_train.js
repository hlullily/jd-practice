...
//缩微图列表向左移动
document.querySelectorAll('#preview a')[0].onclick=function(){
...
//缩微图列表向右移动
document.querySelectorAll('#preview a')[1].onclick=function(){
...
*处理缩略图列表中的图片的鼠标悬停事件
*/
var imgList=document.querySelectorAll('#preview ul img');
for(var i=0;i<imgList.length;i++){
    var img=imgList[i];
    img.onmouseover=changeThumbImg;   
}
function changeThumbImg(){
    var previousHoverd=document.querySelector('.hoveredThumb');
    if(previousHoverd==null){
        this.className='hoveredThumb';
        changeMediumImg(this.src);//修改中等图片的src
    }else if(previousHoverd!=this){
        previousHoverd.removeAttribute('class');
        this.className='hoveredThumb';
        changeMediumImg(this.src);//修改中等图片的src
    }
}
...
*/
document.querySelector('#maskTop').onmousemove=function(event){
    ...
    // }
    // if(h>(this.offsetHeight-h/2)){
    //     top=this.offsetHeight-h;
    // }
    left=x>(this.offsetWidth-w/2)?(this.offsetWidth-w):left;
    top=y>(this.offsetHeight-h/2)?(this.offsetHeight-h):top;
    ...
    largeImg.style.left=-((left*largeImg.width)/this.offsetWidth)+'px';
    largeImg.style.top=-((top*largeImg.height)/this.offsetHeight)+'px';
}
document.querySelector('#maskTop').onmouseover=function(){
    //显示mask
    var mask=document.getElementById('mask');
    ...
    var prdfix=src.substring(0,dotIndex-2);
    var suffix=src.substring(dotIndex);
    src=prdfix+'-l'+suffix;

    var largeImg=document.getElementById('largeImg');
    largeImg.src=src;
    //必须等待图片从服务器加载完成后才能获取它的宽和高
...
document.querySelector('#maskTop').onmouseout=function(){
    //除去mask
    var mask=document.getElementById('mask');
